...
function Widget(props: WidgetProps) {
	...
	const onTabCreated = (tab: Tab) => {
		...
		localStorage.setItem(
			`tab-in-progress-${props.campaign}`,
			JSON.stringify(tab)
		);
		openPaymentForm();
	};
	...
	return (
		<div className="sfua-widget">
			{tab ? (
				<div>
					Processing{' '}
					<button onClick={openPaymentForm}>your donation</button> in
					another window. <br />
					<button onClick={onDonationCancel}>
				...
				<div>
					Collected to date{' '}
					<strong>${campaignData.collected / 100}</strong>
					<DonateForm
